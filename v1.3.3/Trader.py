...
# Function to run the script at a specific time every day
def run_script_at_specific_time(hour, minute):
    while True:
        current_time = datetime.now(pytz.timezone('US/Central')).time()
        if current_time.hour == hour and current_time.minute == minute:
            run_script_daily()
            # Sleep for 24 hours to wait until the next scheduled time
            sleep(24 * 3600)
        else:
            # Sleep for 1 minute and check the time again
            sleep(60)

# Function to run the script
def run_script_daily():
    # Define parameters
    symbol = 'SPY'
    start_date = '2023-09-01'
    end_date = datetime.now().strftime('%Y-%m-%d')  # End date is set to yesterday
...
# Run the script at 7:00 AM Central Time every day
run_script_at_specific_time(7, 0)
